...
import { registerWabaRoutes } from './routes/wabaRoutes'
...
const WONDERPARK_COMPANY_ID = String(process.env.WONDERPARK_COMPANY_ID || '').trim().toLowerCase()
...
function getUserCompanyId(user: any): string | null {
    ...
    if (typeof raw !== 'string') {
        ...
        if (!value) return null
        if (WONDERPARK_COMPANY_ID && value.toLowerCase() !== WONDERPARK_COMPANY_ID) return null
        return value
    }
    const trimmed = raw.trim()
    if (!trimmed) return null
    if (WONDERPARK_COMPANY_ID && trimmed.toLowerCase() !== WONDERPARK_COMPANY_ID) return null
    return trimmed
}
...
httpServer.listen(PORT, async () => {
    console.log(`Wonderpark WABA Server listening on port ${PORT}`)
