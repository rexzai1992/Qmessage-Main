...
export const getLastInboundTs = (
    ...
    const selectedKey = chatId.endsWith('@g.us')
        ? chatId
        : (getCleanId(chatId).replace(/\D/g, '') || getCleanId(chatId).toLowerCase());
    let latestSeconds = 0;
    messages.forEach((msg) => {
        const remoteJid = msg.key?.remoteJid || '';
        const remoteKey = remoteJid.endsWith('@g.us')
            ? remoteJid
            : (getCleanId(remoteJid).replace(/\D/g, '') || getCleanId(remoteJid).toLowerCase());
        if (remoteKey === selectedKey && !msg.key?.fromMe && msg.messageTimestamp) {
    ...
    const contactEntry =
        contacts?.[chatId]
        || contacts?.[`${selectedKey}@s.whatsapp.net`]
        || contacts?.[`${selectedKey}@lid`]
        || contacts?.[selectedKey];
    const contactInbound = contactEntry?.lastInboundAt;
    if (contactInbound) {
