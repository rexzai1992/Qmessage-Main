...
io.on('connection', async (socket) => {
    ...
    socket.on('sendMessage', async (data, ack) => {
        let { profileId, jid, text, media, clientTempId } = data || {}
        ...
        try {
            ...
            if (!messageText && !normalizedMedia) {
                ...
                if (typeof ack === 'function') ack({ success: false, error: 'Type a message or attach media.' })
                return
            }
            ...
            if (typeof ack === 'function') ack({ success: false, error: error?.message || 'Failed to send message' })
        }
    })
